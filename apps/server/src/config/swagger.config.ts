...
const { busHandler, DEFAULT_API_ROUTE } = require('chowbea-axios/api') as {
  busHandler: () => import('express').RequestHandler;
  DEFAULT_API_ROUTE: string;
};
...
export function setupSwagger(
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription(
      'Bank-grade vault API for Aurora Stores. Tokens stay on the BFF.',
    )
    .setVersion('0.1.0')
    .addBearerAuth()
    .build();
